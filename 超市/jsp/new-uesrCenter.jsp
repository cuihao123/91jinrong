...
<div class="content content-use">
	...
	<div class="login" id="oLogin">
		<div class="login-form use-mes clearfix">
			<h4 class="login-t"><em>个人信息</em></h4>
	        <p>
	        	<span class="name-form">登录名：</span>
	        	<span>${baseInfo.mobile}</span>
	        </p>
	        <p>
	        	<span class="name-form">性别：</span>
	        	<span><c:if test="${empty baseInfo.sex}">未知</c:if><c:if test="${not empty baseInfo.sex}">${baseInfo.sex}</c:if></span>
	        </p>
	        <p>
	        ...
	        <p>
	        	...
	        	<span><c:if test="${empty baseInfo.email}">未认证</c:if><c:if test="${not empty baseInfo.email}">${baseInfo.email}</c:if></span>
	        </p>
		</div>
	</div>
	<!-- login-form over -->
