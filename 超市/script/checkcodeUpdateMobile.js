		...
		if(checkMobile(phone,mobileShowId)){
			...
			$.getScript('http://www.91jinrong.com/send/smsInfo?mobile='+phone+"&usefulTime="+usefulTime,function(){
					if("1"==obj['resultcode']){
				                ...
				                setCookie("checkcodemobile",timestamp);
				                setTimeout(run(),1000);
					} 
				});
		}else{
...
function run(){
 	 ...
 	 if(getCookie("checkcodemobile")!="" && (now-getCookie("checkcodemobile"))<(usefulTime*1000)){
 	 	$("#before").hide();
 	 	$("#zhong").hide();
		$("#show_time").show();
		var resutle =usefulTime-parseInt((now-getCookie("checkcodemobile"))/1000);
